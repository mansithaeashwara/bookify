<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "abc123", "bookify");
		if(!$conn){
...
function getAllOrdersWithISBN($conn){
    $query = "SELECT orders.*, order_items.book_isbn FROM orders JOIN order_items ON orders.orderid = order_items.orderid ORDER BY orders.orderid DESC";
    $result = mysqli_query($conn, $query);
    if(!$result){
        return false;
    }
